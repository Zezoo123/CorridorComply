...
- **Sanctions Screening**
  - Combined list from UN, OFAC (SDN and non-SDN consolidated), UK (OFSI consolidated list), EU, **Qatar's NCTC unified record** (domestic designations with QID and passport numbers) and the firm's own internal watchlist, refreshed by `scripts/update_sanctions.py` on a schedule
  - Both parties screened: sender and beneficiary on every corridor decision (QCB 18(9)); country risk from dated FATF lists
