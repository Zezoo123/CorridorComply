...
## Namesakes and shortened name forms

Common names produce most false positives. Two rules keep them out of the way without hiding a real hit:

1. **A hit reached through a shortened form of the name is scored on the name the customer supplied.** The tool also screens shortened forms (a Filipino name without the mother's maiden middle name, a South Asian name without the father's name, the part of a name before "s/o" or "bin"). If such a form matches a list name but the full name scores below the threshold, the hit is kept only when the list entry's date of birth agrees, or, when the entry carries no date of birth and no conflicting nationality, when the shortened form matched almost exactly. A full date of birth that differs on both sides means a namesake and the hit is not shown. The report shows `screened_as` and `variant_similarity` for any hit found this way.
2. **Date of birth and nationality are judged on the strongest candidate, not across all candidates.** A full-name match whose date of birth differs from the entry is demoted to medium and labelled "Date of birth differs from the matching list entry"; it is never hidden. Several weak candidates do not add up to one real match.

Worked example from the demo file: "Muhammad Imran Khan", born 1990, Pakistani. The shortened form "Muhammad Khan" equals the alias "Khan Muhammad" of an Afghan entry born in the 1960s. The full name scores 81 against that alias, the date of birth differs, so the entry is not shown. "Mohammed Rafiqul Islam" against a UN entry "Mohammad Eslami" that carries no date of birth and no nationality is shown at low risk: the data cannot exclude him, and the reviewer clears it with a reason.

## Fewer names than the list holds

Customers often supply two names where the list holds three or four ("Mohammed Mujahid" for a listed "Mohammed Yahya Mujahid"). Ordinary similarity scoring punishes the length difference and misses these. When every name the customer supplied appears on a list entry, exactly or as a transliteration variant, the entry is reported as a `partial` match, scored on the overlap and capped below the high-confidence band, and only when the customer's date of birth agrees with the entry's (exact date or year). Without a date of birth, two common names that happen to appear in a longer listed name are not evidence, and nothing is reported.
